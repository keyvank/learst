...
fn main() {
    ...
    let lin3 = g.alloc(Tensor::<f32>::rand(&mut rng, &[20, 2]));
    let lin3_bias = g.alloc(Tensor::<f32>::rand(&mut rng, &[2]));
    ...
    let out3_bias_sigm = g.call(Softmax::new(), &[out3_bias]);

    let error = g.call(CrossEntropy::new(2, ys), &[out3_bias_sigm]);
